...
//            final File file = new File("\\\\Leviathan\\Movies\\HD\\Batman v Superman L’Aube de la Justice (2016).mkv");
    ...
    private static void printJMetadataVideos(@NonNull final JMetadataVideo jMetadataVideo) {
        ...
        ASPECT_RATIO aRatio = null;
        @Nullable
        final Integer width = jMetadataVideo.getWidth();
        @Nullable
        final Integer height = jMetadataVideo.getHeight();
        if (aspectRatio != null) {
            aRatio = MediaInfoHelper.getClosestRatio(aspectRatio.doubleValue());
        } else if (width != null && height != null) {
            aRatio = MediaInfoHelper.getClosestRatio(width.longValue(), height.longValue());
        }
